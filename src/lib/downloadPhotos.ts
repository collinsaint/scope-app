...
import type { Walk, WalkRoomPhoto } from '../types'
...
export async function downloadSelectedPhotos(
  photos: WalkRoomPhoto[],
  walkName: string,
  projectName: string,
): Promise<void> {
  if (photos.length === 0) return
  const zip = new JSZip()
  const root = zip.folder('selected-photos')!
  const byRoom: Record<string, typeof photos> = {}
  for (const photo of photos) {
    ;(byRoom[photo.room] = byRoom[photo.room] ?? []).push(photo)
  }
  ...
  a.href = url
  a.download = `selected-photos-${walkName.replace(/\s+/g, '-')}-${projectName.replace(/\s+/g, '-')}.zip`
  a.click()

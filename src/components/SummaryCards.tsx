  ...
  if (isMobile) {
    return (
      <div className="flex gap-3 px-4 py-3 overflow-x-auto scrollbar-hide">
        <Card compact label="Total Amount" value={fmt(totalRcv)} sub={`${items.length} items`} />
        ...
        />
        <Card compact label="Remaining" value={fmt(remainingRcv)} sub={`${items.length - completed.length} left`} valueColor={remainingRcv > 0 ? 'text-red-500' : 'text-slate-800'} />
      </div>
    )
  }

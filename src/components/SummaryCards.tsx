...
export function SummaryCards({ items, scopeTotal, opMultiplier = 1 }: Props) {
  ...
  const totalRcv = (scopeTotal ?? billable.reduce((s, i) => s + i.rcv, 0)) * opMultiplier
  const completedRcv = completed.reduce((s, i) => s + i.rcv, 0) * opMultiplier
